...
const AddEnfermedad = () => {
    ...
    const loadProfile = async () => {
        const ProfileResponse = await PerfilMedicoService.getByUserId("628028d16b19388ded66f6be");
    ...
    const getEnfermedades = async (perfil) => {
        for (let i = 0; i < perfil.Enfermedades.length; i++) {
            const EnfermedadResponse = await DiseaseService.get(perfil.Enfermedades[i]);            
    ...
    const onSubmit = async () => {
        if (detectionYear === "" || enfermedad === "") {
            ...
            DiseaseService.register(datos).then((response) => {
                if (response) {
                    ...
                    PerfilMedicoService.update(perfil._id, data).then((response) =>{
